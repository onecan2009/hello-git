...
u8 wifi_custom_mac_addr[6] = {0x00,0x11,0x12,0x13,0x14,0x15};
...
{
	strncpy(user_va,"10:90:4c:11:34:e3",18);
}
printk("user_va is %s,the func is %s\n",user_va,__func__);
			char *p=NULL;
			p=strsep(&user_va,":");
			int a=0;
				while(p)
          {
             wifi_custom_mac_addr[a]=strtol(p,NULL,16);
              p=strsep(&user_va,":");
              a++;
        }
	for(a=0;a<6;a++)
			printk("wifi_custom_mac_add is %02x\n",wifi_custom_mac_addr[a]);
