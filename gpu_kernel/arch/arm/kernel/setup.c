...
#define   XTHREADS_DDR_PHY_SIZE     SZ_1024M
#define   XTHREADS_DDR_PHY_ADRR     (0x80000000 -XTHREADS_DDR_PHY_SIZE)

void xthread_mem_reserve(void)
{
    uint32_t paddr,size;
    paddr = XTHREADS_DDR_PHY_ADRR;
    size = XTHREADS_DDR_PHY_SIZE;
    
   	printk(KERN_INFO "xthread_mem_reserve : begin\n");
    memblock_reserve(paddr,size);
    memblock_free(paddr, size);
    memblock_remove(paddr, size);
	printk(KERN_INFO "xthread_mem_reserve :end \n");
}
/*add over*/
