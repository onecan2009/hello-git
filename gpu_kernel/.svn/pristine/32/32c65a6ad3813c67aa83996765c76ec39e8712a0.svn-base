...
/*************terry 2015-10-29**********************/
struct platform_device *fpdev = NULL;
/****************************************/
...
/***************************************************************/
static ssize_t show_input_pwm(struct device *dev,
...
static ssize_t tsadc_temperature_read(struct file *filp,char *buf,size_t count,loff_t *offp ) 
{
	size_t result;
	ssize_t tmep;
	tmep =  show_input_pwm(&(fpdev->dev),&sensor_dev_attr_temp0_input.dev_attr);
	
	result = copy_to_user(buf, (char *)&tmep, count);
	if (result) {
...
static const struct file_operations tsadc_temperature_fops = {
	.read = tsadc_temperature_read,
	
};

static struct miscdevice tsadc_temperature_device = {
	.minor = MISC_DYNAMIC_MINOR,//255
	.name = "tsadc_temperature",
	.fops = &tsadc_temperature_fops,
};

static int __init tsadc_temperature_init(void)
{
	printk("%s()\n",__FUNCTION__);
	
	return misc_register(&tsadc_temperature_device);
}

static void __exit tsadc_temperature_exit(void)
{
	misc_deregister(&tsadc_temperature_device);
}

module_init(tsadc_temperature_init);
module_exit(tsadc_temperature_exit);
